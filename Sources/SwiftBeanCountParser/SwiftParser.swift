...
enum SwiftParser {
    ...
    private static func parse(lines: [String]) -> Parser.ParsingResult {
        let result = Parser.ParsingResult()
        var lineIndex = -1
    ...
    private static func parseTransaction(at lineIndex: Int, in lines: [String], into result: Parser.ParsingResult) -> Int {
    ...
    private static func sortParsedData(_ result: Parser.ParsingResult) {
    ...
    private static func parse(_ line: String, index lineIndex: Int, metaData: [String: String], into result: Parser.ParsingResult) {
        ...
        if let existingAccount = result.accounts.first(where: { _, _, account in account.name == parsedAccount.name }).map({ _, _, account in account }) {
            if parsedAccount.closing != nil {
                ...
                if existingAccount.opening == nil {
                    result.accounts.removeAll { _, _, account in account.name == parsedAccount.name }
                    let newAccount = Parser.accountFromTemplate(account: existingAccount, opening: parsedAccount.opening)
                    result.accounts.append((lineIndex, line, newAccount))
                } else {
